...
void test_ButtonDebounce_ShouldIgnoreSpikesShorterThan50ms(void) {
	...
	TEST_ASSERT_EQUAL(ST_IDLE, g_SS_State.fsm_state);
	TEST_ASSERT_TRUE(
	    g_SS_State.ss_enable_current); // Sistema deve permanecer ligado
}
...
void test_ButtonToggle_ShouldChangeStateAfterValidPress(void) {
	...
	TEST_ASSERT_FALSE(
	    g_SS_State.ss_enable_current); // Verifica se inverteu o estado
...
void test_Mutation_Killers(void) {
	SS_Init();
	g_SS_Inputs.IgnitionStatus = true;
	g_SS_State.ignition_prev = true;
	g_SS_Inputs.ButtonInput = false;
	SS_Step(); // Sistema em IDLE

	// Inicia o aperto do botão
	...
	// Agora, no próximo passo, a FSM DEVE ter saído de DEBOUNCING
	SS_Step();

	TEST_ASSERT_NOT_EQUAL(ST_DEBOUNCING, g_SS_State.fsm_state);

	// Verifica se o LED de comando foi acionado pela mudança de estado
...
void test_SS_Enabled_Final_Logic(void) {
	...
	SS_Init();
	g_SS_Inputs.IgnitionStatus = true;
	g_SS_State.ignition_prev =
	    true; // Forcei que já estava ligado para evitar borda de subida
	g_SS_State.ss_enable_current = false;
	SS_Step();
	TEST_ASSERT_FALSE(g_SS_Outputs.SS_Enabled_Final);
}
