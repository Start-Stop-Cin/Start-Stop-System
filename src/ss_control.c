...
void SS_Init(void) {
	...
	g_SS_State.cnt = 0;
	g_SS_State.ss_enable_current =
	    true; // Por padrão, Start-Stop inicia Habilitado
...
void SS_Step(void) {
	...
	bool ignition_rising_edge =
	    (g_SS_Inputs.IgnitionStatus && !g_SS_State.ignition_prev);
	...
	g_SS_Outputs.SS_Enabled_Final =
	    g_SS_State.ss_enable_current && g_SS_Inputs.IgnitionStatus;
	g_SS_Outputs.HMI_LED = !g_SS_State.ss_enable_current;
	g_SS_Outputs.LED_Update_Cmd =
	    (g_SS_State.ss_enable_current != ss_enable_prev);
}

void press_SS_button(void) { g_SS_Inputs.ButtonInput = true; }

void release_SS_button(void) { g_SS_Inputs.ButtonInput = false; }
